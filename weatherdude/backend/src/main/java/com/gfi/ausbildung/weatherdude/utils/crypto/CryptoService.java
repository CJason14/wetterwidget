...
/**
 * Utility Klasse rund um Kryptographie.
 *   KeyGenerator
 *   Konvertierungen für Schlüssel
 *   Verschlüsselung
 */
@Component
@Slf4j
public class CryptoService
{
  ...
  public PublicKey convertX509ToECPublicKey(byte[] encodedPublicKey) throws InvalidKeySpecException
  {
    X509EncodedKeySpec pubX509 = new X509EncodedKeySpec(encodedPublicKey);
    return this.keyFactory.generatePublic(pubX509);
  }

  public PrivateKey convertPKCS8ToECPrivateKey(byte[] encodedPrivateKey) throws InvalidKeySpecException
  {
  ...
  public ECPublicKey fromUncompressedECPublicKey(String encodedPublicKey) throws InvalidKeySpecException
  {
  ...
  public byte[] encrypt(String plainTextString, String uaPublicKeyString, String authSecret, int paddingSize)
      throws InvalidKeyException, NoSuchAlgorithmException, InvalidKeySpecException, InvalidAlgorithmParameterException,
      NoSuchPaddingException, IllegalBlockSizeException, BadPaddingException
  {
    ...
    byte[] keyInfo = concat("WebPush: info\0".getBytes(StandardCharsets.UTF_8), toUncompressedECPublicKey(uaPublicKey),
    ...
    byte[] header = concat(salt, encryptedArrayLength.array(), new byte[] { (byte) uncompressedASPublicKey.length },
        uncompressedASPublicKey);
